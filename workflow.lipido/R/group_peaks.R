...
group_peaks <- function(xsets, pd_params, operator, pb_fct = NULL) {
    peaks <- xsets@peaks
    ...
    ## value >= mass[i].
    mass <- seq(peaks[1, "mz"], peaks[nrow(peaks), "mz"] +
                  pd_params@binSize,
    ...
    if (nrow(groups) > 0) {
        ...
        numsamp <- rowSums(as.matrix(groups[, (match(
            "npeaks", colnames(groups)) + 1):(ncol(groups) - 1),
            drop = FALSE]))
        uorder <- order(-numsamp, groups[, "npeaks"])

        uindex <- xcms:::rectUnique(as.matrix(groups[, c(
            "mzmin", "mzmax","rtmin", "rtmax"), drop = FALSE]), uorder)
