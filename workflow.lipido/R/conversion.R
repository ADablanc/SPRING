...
#' @examples
#' \dontrun{
#' convert_files("testdata/small.RAW",
...
convert_files <- function(raw_files,
  ...
  conversion_res <- operator(
    ...
    cbind(
      ...
      pos = tryCatch(
        convert_file(
          raw_file, pos_outdir, converter, "positive",
          cores == 1
        ),
        error = function(e) {
          e$message
      ...
      neg = tryCatch(
        convert_file(
          raw_file, neg_outdir, converter, "negative",
          cores == 1
        ),
        error = function(e) {
          e$message
